...
class Hyperband:
    ...
    def run(self, skip_last=0, dry_run=False, fixed_params=None):
        ...
        data_train = dm.LoadData(args=fixed_params,interval=fixed_params.train_interval)   
        data_validate = dm.LoadData(args=fixed_params,interval=fixed_params.validate_interval)
        data_test = dm.LoadData(args=fixed_params,interval=fixed_params.test_interval)
            ...
            for i in range((s + 1) - (int(skip_last)-1)):  # changed from s + 1
                ...
                for t in T:
                    ...
                    if dry_run:
                        ...
                        result = self.try_params(n_iterations, t, fixed_params,data_train,data_validate,data_test)  # <---
