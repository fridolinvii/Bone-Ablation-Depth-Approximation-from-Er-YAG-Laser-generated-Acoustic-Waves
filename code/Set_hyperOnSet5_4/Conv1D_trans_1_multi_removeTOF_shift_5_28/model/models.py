...
class ParametricConvFCModel(nn.Module):
    ...
    def __init__(self, image, args, params):
        ...
        self.layer_extras = [{'D': rate} if rate is not None else None for rate in dropout_rates] +[None]
        ###################################################################################
        ###################################################################################
        ## Parameters ##
...
#            batchnorm = nn.InstanceNorm1d(channels_in[0])
...
#                    batchnorm = nn.InstanceNorm1d(channels_out[i])
...
#                elif self.layer_types[i - 2] == 'C': # to add start_pos
#                        fc_input_size += 1
                ...
                if i != len(self.layer_types) - 1 and dropout_rates[i] is not None:
                    ## add dropout
                    self.seq_layers[str(j)] = nn.Dropout(p=dropout_rates[i])
                    j = j + 1
                if i != len(self.layer_types) - 1:
                    ## FC same size as output of Conv
                    if fc_input_size>neurons:  #reduces dimension
                        channels_out_ = neurons
                    else:
                        channels_out_ = fc_input_size
                else:
                    ## FC to classifyer
                    channels_out_ = 1 #675 #len(self.args.classes)
...
#                    batchnorm = nn.InstanceNorm1d(channels_out_)
        ...
        if self.error is True:
          ...
          for i in range(len(self.layer_types)):
            ...
            if self.layer_types[i] == 'F' and self.layer_types[i - 1] == 'C':
                ...
                x_cnn = x
                x_cnn = x_cnn/th.norm(x_cnn,dim=-1,keepdim=True)

#            elif self.layer_types[i] == 'F' and self.layer_types[i - 2] == 'C':
#                ## add start_pos to x
#                x = th.cat([x,start_pos.unsqueeze(-1)],dim=-1)

            if self.layer_extras[i] is not None and 'D' in self.layer_extras[i]: # add dropout
                ## Dropout ##
                layer=self.seq_layers[str(j)]
                j += 1
                x=layer(x)

            if i < len(self.layer_types) - 1 and self.layer_types[i] is not 'C':
                ## FC layer ##
                layer = self.seq_layers[str(j)]
                x = layer(x)
                j += 1
