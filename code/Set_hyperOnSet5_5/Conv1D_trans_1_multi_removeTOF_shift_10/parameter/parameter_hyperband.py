...
def get_space():
        ...
        for i in range(1, layers_conv_range[1] + 1):  
            ...
            space['conv_layer_{}_strides'.format(i)] = 1 #hp.quniform('ls{}'.format(i), stride_range[0],stride_range[1],stride_range[2])
            space['conv_layer_{}_kernel_size_maxpool'.format(i)] = hp.quniform('mp{}'.format(i), maxpool_range[0],maxpool_range[1],maxpool_range[2])
            space['conv_layer_{}_strides_maxpool'.format(i)] = 1 # hp.quniform('mp{}'.format(i), maxpool_stride_range[0],maxpool_stride_range[1],maxpool_stride_range[2])
